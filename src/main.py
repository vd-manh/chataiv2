...
def render_assistant_response(ans, cits, vids):
    ...
    with t1: st.markdown(ans)
    with t2:
        if cits:
            for c in cits: st.caption(f"📌 **{c.get('filename')}** - {c.get('page')}페이지")
        else: st.write(LANG["no_src"])
    with t3:
        if vids:
        ...
        with st.chat_message("assistant"):
            with st.spinner(LANG["status_search"]):
                ...
                try:
                    ...
                    if t and l: current_vids = [(t[i], l[i]) for i in range(min(3, len(t)))]
                except: pass
